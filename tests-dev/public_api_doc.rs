...
//! (`docs/public-api/zenblend.txt`) on every `cargo test` run, so API
//! changes always show up as a git diff next to the code change that caused
//! them, and the surface size stays one glance away.
//!
//! Lives in `tests-dev/` (outside the `include` whitelist) with an explicit
//! `[[test]]` block in Cargo.toml, so the published crate ships neither the
//! test nor its cargo-public-api/nightly-rustdoc requirement.
//!
...
fn public_api_surface_doc_is_current() {
    ...
    doc.push_str(&format!(
        "# {CRATE} public API surface\n\
         # Generated by tests-dev/public_api_doc.rs via `cargo public-api --simplified`\n\
