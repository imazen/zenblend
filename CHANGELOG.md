...
### Documentation
- Recorded an unresolved numerics divergence in `SrcOver` at both sites a reader would hit it: the x86_64 kernel `blend_src_over_row_v3` uses a fused multiply-add for its vector body while **its own scalar tail** and the portable `f32x4` kernel use an unfused `fg + bg * inv_alpha`, so it can differ from itself by up to 1 ULP on the last pixel of an odd-length row and from the NEON/WASM/scalar backends everywhere. That is why `SrcOver` is the one row kernel missing from `porter_duff_simd_matches_scalar_bit_for_bit`. Not changed here — picking a side is a cross-backend decision with a perf cost either way — but no longer undocumented.
